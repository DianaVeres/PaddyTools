...
export async function GET() {
  ...
  const historicalSchools = historicalPackingLists.map((item) => item.school.trim()).filter(Boolean);
  try {
    ...
    const schools = [...new Set([...((await response.json()) as Array<{ name: string }>)
    ...
    return NextResponse.json([...new Set(historicalSchools)].sort((a, b) => a.localeCompare(b, 'es')));
  }
}
